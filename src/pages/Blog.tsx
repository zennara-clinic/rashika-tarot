...
import { Calendar, User, ArrowRight, Search, Sparkles, X } from "lucide-react";
...
const Blog = () => {
  ...
  const categories = ["All", "Tarot", "Astrology", "Numerology", "Vastu", "Palmistry", "Spiritual Practices"];
      ...
      <section className="relative z-10 pt-32 pb-16 px-4">
        <div className="container mx-auto text-center">
          ...
          <p className="text-xl text-white/80 max-w-3xl mx-auto mb-8">
            Explore our collection of articles on Tarot, Astrology, Numerology, Vastu, and spiritual practices to deepen your understanding and enhance your journey.
          </p>
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-12 py-4 bg-white/10 backdrop-blur-md border-2 border-white/20 rounded-none text-white placeholder-white/50 focus:outline-none focus:border-yellow-400 transition-colors text-lg"
              />
              {searchQuery && (
                <button
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            {categories.map((category) => (
              <button
                ...
                className={`px-6 py-3 rounded-none font-semibold transition-all border-2 ${
                  selectedCategory === category
                    ? 'bg-gradient-to-r from-yellow-400 to-yellow-500 text-black border-yellow-400 shadow-[0_0_20px_rgba(251,191,36,0.5)]'
                    : 'bg-white/5 backdrop-blur-md text-white border-white/20 hover:bg-white/10 hover:border-white/40'
                }`}
              >
                {category}
              </button>
          ...
          <div className="mt-6">
            <p className="text-white/70 text-lg">
              Showing <span className="text-yellow-400 font-semibold">{filteredPosts.length}</span> {filteredPosts.length === 1 ? 'article' : 'articles'}
            </p>
          </div>
        </div>
      </section>
          ...
          {filteredPosts.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPosts.map((post) => (
                <Card key={post.id} className="bg-white/10 backdrop-blur-md border-2 border-white/20 rounded-none overflow-hidden hover:shadow-2xl hover:shadow-yellow-400/20 transition-all duration-300 group hover:scale-[1.02]">
                  {/* Image */}
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3">
                      <span className={`bg-gradient-to-r ${getCategoryColor(post.category)} text-white text-xs font-bold px-3 py-1 rounded-none shadow-lg`}>
                        {post.category}
                      </span>
                    </div>
                    ...
                    <div className="absolute bottom-3 right-3">
                      <span className="bg-black/70 backdrop-blur-sm text-white text-xs px-2 py-1 rounded-none">
                        {post.readTime}
                      </span>
                    </div>
                  </div>
                  ...
                  <CardContent className="p-4">
                    ...
                    {/* Title */}
                    <h3 className="text-lg font-bold text-white mb-2 group-hover:text-yellow-400 transition-colors leading-tight line-clamp-2">
                      {post.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-white/20">
                      <Link to={`/blog/${post.id}`}>
                        <Button 
                          variant="ghost" 
                          className="text-yellow-400 hover:text-yellow-300 hover:bg-yellow-400/10 font-semibold group/btn rounded-none px-0 text-sm"
                        >
                          Read Article
            ...
            <div className="text-center py-20">
              <div className="bg-white/5 backdrop-blur-md border-2 border-white/20 rounded-none p-12 max-w-md mx-auto">
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-yellow-400 to-yellow-500 text-black font-semibold px-8 py-3 rounded-none hover:shadow-[0_0_30px_rgba(251,191,36,0.6)]"
                >
                  Clear Filters
                </Button>
              </div>
